...
impl InGameWindow for MakeItemWindow {
    fn build(
        ...
        let win_h = TITLE_H + PAD + list_h + PAD + SLOT_AREA_H + FOOTER_H;
        ...
        ui.text(dx + 17.0, dy + TITLE_H - 3.0, "Create Medicine", tc);

        let body_y = dy + TITLE_H;
        let body_h = PAD + list_h + PAD + SLOT_AREA_H;
        ...
        // Material slots (Star Crumb / elemental stone).
        let slots_y = list_y + list_h + PAD;
        for s in 0..3 {
            let slot_x = dx + PAD + s as f32 * (SLOT_SIZE + PAD);
            let slot_rect = Rect::new(slot_x, slots_y, SLOT_SIZE, SLOT_SIZE);
            if grf {
                let (v, i) = draw::quad_vertices(slot_x, slots_y, SLOT_SIZE, SLOT_SIZE, [1.0; 4]);
                ui.draw_calls.push(DrawCall {
                    vertices: v.to_vec(),
                    indices: i.to_vec(),
                ...
                let (v, i) =
                    draw::quad_vertices(slot_x, slots_y, SLOT_SIZE, SLOT_SIZE, [0.1, 0.1, 0.15, 0.9]);
                ui.draw_calls.push(DrawCall {
                    vertices: v.to_vec(),
                    indices: i.to_vec(),
                    texture: TextureRef::White,
            ...
            // Accept a drop from the inventory window.
...
mod tests {
    ...
    #[test]
    fn open_populates_rows_and_builds_without_panicking() {
