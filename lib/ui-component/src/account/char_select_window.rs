...
impl CharSelectWindow {
    ...
    pub fn build(&mut self, ui: &mut UiFrame) -> Vec<GameEvent> {
        ...
        {
            events.push(GameEvent::RequestSelectCharacter {
                slot: ch.slot as u8,
            });
        }
    ...
    fn build_fallback(&mut self, ui: &mut UiFrame, events: &mut Vec<GameEvent>) {
        ...
        let padding = 8.0;
        let title_h = 30.0;
        let detail_h = if self.selected_index.is_some() {
            100.0
        } else {
            0.0
        };
            ...
            && let Some(ch) = self.characters.get(idx)
        {
            ...
            ui.text(
                col1_x,
                y,
                &format!("HP: {}/{}", ch.hp, ch.max_hp),
                dim_color,
            );
            ui.text(
                col2_x,
                y,
                &format!("SP: {}/{}", ch.sp, ch.max_sp),
                dim_color,
            );
            ...
            ui.text(
                col1_x,
                y,
                &format!("STR {} AGI {} VIT {}", ch.str, ch.agi, ch.vit),
                dim_color,
            );
            ...
            ui.text(
                col1_x,
                y,
                &format!("INT {} DEX {} LUK {}", ch.int, ch.dex, ch.luk),
                dim_color,
            );
        }
