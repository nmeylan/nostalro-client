    ...
    for quad in quads {
        let bind_group = match quad.source {
            TextureSource::Number => {
                if quad.tex_idx >= num_textures.bind_groups.len() { continue; }
            ...
            TextureSource::Message => {
                ...
                if quad.tex_idx >= mt.bind_groups.len() { continue; }
                &mt.bind_groups[quad.tex_idx]
            }
        };
        let idx = inline_textures.len();
        inline_textures.push(bind_group);
        draw_calls.push(textured_quad(quad.x, quad.y, quad.w, quad.h, quad.color, idx));
    }
}
